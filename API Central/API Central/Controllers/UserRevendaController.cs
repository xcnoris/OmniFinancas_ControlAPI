...
namespace API_Central.Controllers
{
    ...
    public class UserRevendaController : ControllerBase
    {
        ...
        public UserRevendaController(
            DAL<RevendaModel> dalRevenda,
            DAL<UsuariosRevendaModel> dalrevenda
            )
        {
            _dalRevenda = dalRevenda;
            _dalUserRevenda = dalrevenda;
        }
        ...
        public async Task<ActionResult<UsuariosRevendaModel>> CriarUserRevenda([FromBody] DTOUserRevenda UserRevendaRequest)
        {
            try
            {
                RevendaModel? revendaExistente = await _dalRevenda.RecuperarPorAsync(x => x.Id.Equals(UserRevendaRequest.RevendaId));
                if (revendaExistente is null) return BadRequest($"Nao foi encontrado nenhuma Revenda com o ID: {UserRevendaRequest.RevendaId}");
                ...
                await _dalUserRevenda.AdicionarAsync(NovoUserRevenda);

                // Retornar a revenda criada
                return Ok(NovoUserRevenda);
            }
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<UsuariosRevendaModel>>> BuscarTodos()
        {
            try
            {
                IEnumerable<UsuariosRevendaModel> revendas = await _dalUserRevenda.ListarAsync();
                return Ok(revendas);
            }
            catch (ValidationException ex)
            {
